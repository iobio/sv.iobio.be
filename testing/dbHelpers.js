...
export function getGeneAssociations(genes, db) {
    ...
    return new Promise((resolve, reject) => {
        getGeneIdsList(genes, db).then((geneIdsList) => {
            ...
            Promise.all([getPhenotypes(), getDiseases()])
                .then(([phenotypeMap, diseaseMap]) => {
...
export function getOverlappedGenes(build, source, startChr, startPos, endChr, endPos, sourceText, db) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (startChr == endChr) {
            ...
            query =
                `SELECT * FROM genes WHERE ${buildText} AND chr = ? AND ((start >= ? AND end <= ?) OR ((? >= start AND ? < end) OR (? >= start AND ? < end)))` +
                sourceText;
        } else {
            params = [startChr, startPos, startPos, endChr, endPos, endPos];
            query =
                `SELECT * FROM genes WHERE ${buildText} AND ((chr = ? AND (start >= ? OR end >= ?)) OR (chr = ? AND (end <= ? OR start <= end ?))` +
                sourceText;
        }
